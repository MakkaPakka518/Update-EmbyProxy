...
const GITHUB_RAW_URL = "https://raw.githubusercontent.com/MakkaPakka518/Update-EmbyProxy/refs/heads/main/double/worker1.js";
        ...
        <div class="card" style="max-width: 1000px; margin: 40px auto; position:relative; box-shadow: 0 10px 40px rgba(0,0,0,0.2);">
            ...
            <h2 style="margin-top:0; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                ...
                <div style="font-size: 13px; background: rgba(0,113,227,0.1); color: var(--primary); padding: 6px 12px; border-radius: 8px; border: 1px solid rgba(0,113,227,0.2); display: flex; gap: 15px; flex-wrap: wrap;">
                    <span>🌐 当天: <strong id="trafficToday">加载中...</strong></span>
...
// 用于生成 TG 播报消息的核心工具函数 (双面板联合播报版 + 流量之王统计)
async function sendTgStats(env, chatId) {
    try {
        // ================= 获取 面板 1 数据 =================
        const p1Total = await env.DB.prepare(`SELECT COUNT(*) as count FROM visitor_logs WHERE date(timestamp, '+8 hours') = date('now', '+8 hours')`).first();
        const p1Reg = await env.DB.prepare(`SELECT country, COUNT(*) as c FROM visitor_logs WHERE date(timestamp, '+8 hours') = date('now', '+8 hours') GROUP BY country ORDER BY c DESC LIMIT 1`).first();
        const p1Node = await env.DB.prepare(`
            SELECT r.remark, COUNT(v.id) as c FROM visitor_logs v LEFT JOIN routes r ON v.prefix = r.prefix 
            WHERE date(v.timestamp, '+8 hours') = date('now', '+8 hours') GROUP BY v.prefix ORDER BY c DESC LIMIT 1
        `).first();

        const p1Count = p1Total ? p1Total.count : 0;
        const p1RegStr = p1Reg ? `${p1Reg.country === 'CN' ? '🇨🇳 中国大陆' : p1Reg.country} (${p1Reg.c}次)` : '无记录';
        const p1NodeStr = p1Node ? `${p1Node.remark || '未命名'} (${p1Node.c}次)` : '无记录';

        // ================= 获取 面板 2 数据 =================
        let p2Count = 0; let p2RegStr = '未绑定或无记录'; let p2NodeStr = '未绑定或无记录';
        if (env.DB2) { 
            const p2Total = await env.DB2.prepare(`SELECT COUNT(*) as count FROM visitor_logs WHERE date(timestamp, '+8 hours') = date('now', '+8 hours')`).first();
            const p2Reg = await env.DB2.prepare(`SELECT country, COUNT(*) as c FROM visitor_logs WHERE date(timestamp, '+8 hours') = date('now', '+8 hours') GROUP BY country ORDER BY c DESC LIMIT 1`).first();
            const p2Node = await env.DB2.prepare(`
                SELECT r.remark, COUNT(v.id) as c FROM visitor_logs v LEFT JOIN routes r ON v.prefix = r.prefix 
                WHERE date(v.timestamp, '+8 hours') = date('now', '+8 hours') GROUP BY v.prefix ORDER BY c DESC LIMIT 1
            `).first();

            p2Count = p2Total ? p2Total.count : 0;
            p2RegStr = p2Reg ? `${p2Reg.country === 'CN' ? '🇨🇳 中国大陆' : p2Reg.country} (${p2Reg.c}次)` : '无记录';
            p2NodeStr = p2Node ? `${p2Node.remark || '未命名'} (${p2Node.c}次)` : '无记录';
        }

        // ================= 获取主域名共享流量 =================
        const [trafficToday, traffic7d, traffic30d] = await Promise.all([
            getCFTraffic(env, 'today'), getCFTraffic(env, 7), getCFTraffic(env, 30)
        ]);
    ...
    async fetch(request, env, ctx) {
        ...
        if (url.pathname === '/api/deploy' && request.method === 'POST') {
            ...
            try {
                ...
                // 2. 🚀 核心修复：通过 CF API 拉取 D1 数据库等高级绑定并无损合并
                ...
                if (bindingsData.success && Array.isArray(bindingsData.result)) {
                    for (const b of bindingsData.result) {
                        // 排除已经备份过的纯文本，专抓 D1、KV 这些特殊对象，原封不动保留
                ...
                const metadata = { 
                    ...
                    bindings: preservedBindings // 把完整的文本变量 + D1数据库配置一起塞回去
                };
                ...
                const data = await res.json();
                if (data.success) {
                    return Response.json({ success: true, msg: '代码、环境变量与 D1 数据库均已成功无损更新！' });
                } else {
        ...
        // ==========================================
        // 🚀 修复版 302 拦截：把节点前缀拼回去，防止下一次请求报 404
        // ==========================================
        if ([301, 302, 303, 307, 308].includes(finalResponse.status)) {
            const location = responseHeaders.get('Location');
            if (location && /^https?:\/\//i.test(location)) {
                // 如果 matchedPrefix 存在（比如 123），就拼成 /123/https://...
                // 这样下一次请求进来时，前端的 prefix 依然是 123，就能正确命中 DB 节点！
                const prefixPath = matchedPrefix ? `/${matchedPrefix}` : '';
                responseHeaders.set('Location', `${prefixPath}/${location}`);
            }
        }
        ...
        // 🔄 恢复原版精准 JSON 解析，绝不破坏 Emby 核心内部参数
        if (finalResponse.status === 200 && responseHeaders.get("content-type")?.includes("json") && url.pathname.toLowerCase().includes("playbackinfo")) {
            try {
                let clonedRes = finalResponse.clone(); let data = await clonedRes.json(); let modified = false;
                    ...
                    data.MediaSources.forEach(source => {
                        ['DirectStreamUrl', 'TranscodingUrl'].forEach(key => {
                            if (source[key] && source[key].startsWith('http')) { 
                                source[key] = proxyOrigin + prefixStr + '/' + source[key]; 
                                modified = true; 
                            }
                        });
                    });
        ...
        if (finalResponse.status === 200 && url.pathname.toLowerCase().endsWith('.m3u8')) {
            try {
                ...
                if (text.includes('http://') || text.includes('https://')) {
                    let modifiedText = text.replace(/(https?:\/\/[^\s]+)/g, proxyOrigin + prefixStr + '/$1');
                    responseHeaders.delete("Content-Length"); 
        ...
        const isStaticRes = /\.(jpg|jpeg|gif|png|svg|ico|webp|js|css|woff2?|ttf|otf|map|webmanifest|srt|ass|vtt|sub)$/i.test(url.pathname) || /(\/Images\/|\/Icons\/|\/Branding\/|\/emby\/covers\/)/i.test(url.pathname);
            ...
            responseHeaders.set('Cache-Control', 'public, max-age=86400'); responseHeaders.delete('Expires'); responseHeaders.delete('Pragma');  
